...
<head>
  ...
  <script>
    ...
    // תחנות: מאפשר גם "תחנת X" וגם "X"
    function stationNameVariants(name){
      if(!name) return [];
      const raw = String(name).trim();
      const withoutPrefix = raw.replace(/^תחנת\s+/g, "").trim();
      return [raw, withoutPrefix].map(normalizeKey).filter(Boolean);
    }
    ...
    function updateMetricButtonsUI() {
      ...
      if (headerP) {
        let modeText = "ישובים/רשויות";
        if (currentMode === "merhav") modeText = "מרחבים";
        if (currentMode === "station") modeText = "תחנות";
        headerP.innerHTML = rateAvailable
          ? `מצב נוכחי: <strong>${modeText}</strong><br>ל-10,000 זמין רק כאן`
          : `מצב נוכחי: <strong>${modeText}</strong><br>ל-10,000 לא זמין כאן`;
      }
    ...
    // -------------------- Station dataset resolver --------------------
    // חשוב: אם בקובץ תחנות הגדרת const crimeByYearStations / CRIMEBYYEARSTATIONS (const גלובלי),
    ...
    function getStationsDataset(){
      ...
      try{
        // eslint-disable-next-line no-undef
        if (typeof crimeByYearStations !== "undefined" && crimeByYearStations) return crimeByYearStations;
      }catch(_){}
      try{
        // eslint-disable-next-line no-undef
        if (typeof CRIMEBYYEARSTATIONS !== "undefined" && CRIMEBYYEARSTATIONS) return CRIMEBYYEARSTATIONS;
    ...
    function getDatasetByMode(mode){
      if (mode === "muni") return window.crimeByYear;
      if (mode === "merhav") return window.crimeByYearMerhav;
      return getStationsDataset();
    }

    function calculateStats(year, mode = currentMode) {
      const ds = getDatasetByMode(mode);
      const yearData = ds?.[year];
    ...
    function updateQuickStats() {
      const stats = yearlyStats[`${currentMode}_${currentYear}`] || { municipalities: 0, total: 0, avg: 0 };
    ...
    function buildMuniIndexForYear(year) {
      ...
      yearlyStats[`muni_${year}`] = calculateStats(year, "muni");
    ...
    function buildMerhavIndexForYear(year) {
      ...
      yearlyStats[`merhav_${year}`] = calculateStats(year, "merhav");
      console.log(`✅ Year ${year}: ${Object.keys(merhavIndex).length} merhav keys indexed`);
    }

    function buildStationIndexForYear(year) {
      for (const k in stationIndex) delete stationIndex[k];
      const yearData = getStationsDataset()?.[year];
      if (!yearData) {
        console.warn(`⚠️ No station data for year ${year} (crimeByYearStations)`);
        return;
      }
      ...
      for (const name in yearData) {
        ...
        for (const v of vars) if (!stationIndex[v]) stationIndex[v] = entry;
      }
      yearlyStats[`station_${year}`] = calculateStats(year, "station");
      console.log(`✅ Year ${year}: ${Object.keys(stationIndex).length} station keys indexed`);
    }
    ...
    function entityLabelByMode(mode){
    ...
    function switchTab(tabName) {
      ...
      const entityType = entityLabelByMode(currentMode);

      if (tabName === "single") {
        showInfo(
          "מפה לפי " + modeLabel(currentMode),
          `שנה נבחרת: ${currentYear}<br>לחץ על ${entityType} במפה כדי לראות נתונים`,
          "info"
        );
        ...
        if (!selectedCity) {
          if (tc) tc.innerHTML = `<div class="message message-info">בחר ${entityType} במפה כדי לראות נתונים היסטוריים</div>`;
          return;
        }
    ...
    function setMode(mode) {
      ...
      const layersToHide = [MUNICIPAL_LAYER, POLICE_MERHAV_LAYER, POLICE_STATION_LAYER].filter(x => x !== layerToShow);
      ...
      // סטטיסטיקות מהדאטה של אותו מצב/שנה
      ...
      if (currentTab === "compare") updateCompareView();
        ...
        const tc = document.getElementById("trends-content");
        if (tc) tc.innerHTML = `<div class="message message-info">בחר ${entityLabelByMode(mode)} במפה כדי לראות נתונים היסטוריים</div>`;
      } else {
        showInfo(
          "מפה לפי " + modeLabel(mode),
          `שנה נבחרת: ${currentYear}<br>לחץ על ${entityLabelByMode(mode)} במפה כדי לראות נתונים`,
          "info"
        );
      }
    }
    ...
    function setMetric(metric) {
      ...
      if (currentTab === "single" && selectedCityKey) {
        const index = (currentMode === "muni") ? muniIndex : (currentMode === "merhav") ? merhavIndex : stationIndex;
    ...
    function addToCompare() {
      if (!selectedCity) {
        alert(`אנא בחר ${entityLabelByMode(currentMode)} במפה תחילה`);
        return;
      }
    ...
    function updateCompareView() {
      ...
      if (currentMode !== "muni") {
        html += `<div class="message message-info">במצב ${escapeHtml(modeLabel(currentMode))} ההשוואה היא לפי כמות בלבד.</div>`;
      ...
      sorted.forEach(r => {
        ...
        const pct = (Number(value) / maxVal) * 100;
        const label = metricIsRate ? `${Number(value).toLocaleString()} /10k` : Number(value).toLocaleString();
    ...
    function showTrends(entityName, mode = currentMode) {
      ...
      if (!entityName) {
        container.innerHTML = `<div class="message message-info">בחר ${entityLabelByMode(mode)} במפה כדי לראות נתונים היסטוריים</div>`;
        return;
      }
      ...
      if (!dataset || typeof dataset !== "object") {
        container.innerHTML = `<div class="message message-error">אין נתונים זמינים עבור מצב זה.</div>`;
        return;
      }
      ...
      const tryMatchInYear = (yearData) => {
        ...
        if (mode === "station") {
          const vars = stationNameVariants(entityName);
          for (const k of keys) {
            const nk = normalizeKey(k);
            if (vars.includes(nk)) return { matchedName: k, data: yearData[k] };
          }
        }
      ...
      if (mode === "muni" && currentMetric === "rate") {
        const popRaw = getPopulationForAuthority(entityName);
        const pop = (popRaw == null ? 0 : Number(popRaw));
        series = yearlyData.map(d => {
          const rate = calcRatePer10k(d.total, pop);
          return { year: d.year, value: rate, label: `${rate.toLocaleString()} /10k` };
        });
      }
      ...
      if (missingYears.length) {
        html += `<div class="message message-warn">אין נתונים עבור ה${entityLabelByMode(mode)} בשנים: <strong>${missingYears.join(", ")}</strong></div>`;
      }

      if (mode === "muni" && currentMetric === "rate") {
        const popRaw = getPopulationForAuthority(entityName);
        const pop = (popRaw == null ? 0 : Number(popRaw));
        html += `<div class="message message-info">אוכלוסייה (2019): ${Number(pop).toLocaleString()} (חסר = 0)</div>`;
      }
    ...
    function detectStationName(attrsRaw) {
      const attrs = fieldsToObject(attrsRaw) || attrsRaw;
      const candidates = [
        "שם תחנה", "שם תחנת משטרה",
        "STATION", "SHEM_TAHANA", "TAHANA",
        "NAME", "Name"
      ];
      for (const c of candidates) if (attrs && attrs[c]) return attrs[c];
      return null;
    ...
    async function onMapClick(e) {
      ...
      try {
        ...
        if (currentMode === "station") {
          ...
          const entry = stationIndex[key] || stationIndex[stationNameVariants(stationName)[0]];
          if (!entry) {
          ...
          selectedCity = entry.name;
          selectedCityKey = stationNameVariants(entry.name)[0];
    ...
    function renderRankings() {
      ...
      const yearData =
        (currentMode === "muni") ? window.crimeByYear?.[currentYear] :
        (currentMode === "merhav") ? window.crimeByYearMerhav?.[currentYear] :
        window.crimeByYearStations?.[currentYear];
      ...
      const header = `
        <div class="message message-info" style="margin-bottom:10px;">
          שנה: <strong>${currentYear}</strong> • מצב: <strong>${escapeHtml(modeLabel(currentMode))}</strong><br>
          דירוג אוטומטי לפי: <strong>${metricIsRate ? "ל-10,000" : "כמות (אבסולוטי)"}</strong> • Top ${topN}
        </div>
      `;
      ...
      let note = "";
      if (metricIsRate) {
        note = `<div class="message message-warn">דירוג ל-10,000 משתמש באוכלוסייה (2019); חסר אוכלוסייה הונח כ-0.</div>`;
      } else if (currentMode !== "muni") {
        note = `<div class="message message-info">במצב ${escapeHtml(modeLabel(currentMode))} הדירוג הוא לפי כמות (אין אוכלוסייה).</div>`;
      }
      ...
      top.forEach((r, idx) => {
        const sub = (metricIsRate && r.extra)
        ...
        const valueText = metricIsRate
          ? `${Number(r.value || 0).toLocaleString()} /10k`
    ...
    // ✅ ניקוי יציב: מנקה רק גיאומטריות, לא נוגע באירועי CLICK
    ...
    function setChoroBtnUI(){
      ...
      // כפתור צביעה פעיל רק במצב רשויות
      const allow = (currentMode === "muni");
      btn.classList.toggle("disabled", !allow);
      btn.disabled = !allow;

      if(!allow){
        btn.textContent = "🎨 צביעה (רשויות בלבד)";
        btn.title = "צביעה זמינה רק במצב ישובים/רשויות";
        return;
      }

      if(!muniBoundariesLoaded){
        btn.textContent = "🎨 צביעה";
        ...
        const metricText = currentMetric === "rate" ? "ל-10,000" : "כמות";
        btn.textContent = choroplethEnabled ? `🎨 צביעה (${metricText}): פעיל` : `🎨 צביעה (ירוק→אדום)`;
        btn.title = `צביעה לפי ${metricText}`;
      }
    }

    async function toggleChoropleth(){
      // צביעה רק במצב רשויות
      ...
      if(!ok){
        ...
        showInfo(
          "לא הצלחתי לצבוע",
          `לא ניתן לטעון את קובץ הגבולות.<br>ודא שהוא ליד <code>index.html</code> ושאתה פותח דרך שרת (לא file://).`,
          "error"
        );
        return;
      }
    ...
    async function renderChoropleth(){
      ...
      const refValues = normalRows.map(r => r.value);
      const sortedValues = [...refValues].sort((a, b) => a - b);

      const minVal = sortedValues[0] || 0;
      const maxVal = sortedValues.length >= 2 ? sortedValues[sortedValues.length - 2] : (sortedValues[sortedValues.length - 1] || 0);

      const wkts = [], names = [], symbols = [], tooltips = [];
      const nameToMuniMap = {}; // ✅ תיקון: מיפוי גיאומטריה->שם רשות

      // רגיל (ירוק→אדום)
      for(let i = 0; i < normalRows.length; i++){
        const r = normalRows[i];
        const cappedValue = Math.min(r.value, maxVal);
        const [rr, gg, bb] = rateToColor(cappedValue, minVal, maxVal);
        ...
        const tooltipText = (currentMetric === "rate")
          ? `${r.name} • ${r.rate.toLocaleString()} /10k`
          : `${r.name} • ${r.total.toLocaleString()} אירועים`;
      ...
      for(let i = 0; i < noJurRows.length; i++){
        const r = noJurRows[i];
        wkts.push(r.wkt);
        const nm = `${CHORO_NAME}_nojur_${i}`;
        names.push(nm);
        nameToMuniMap[nm] = r.name;
      ...
      try {
        ...
        // ✅ קליק על גיאומטריות צביעה (Deferred.progress)
        try {
      ...
      const metricTitle = (currentMetric === "rate") ? "צביעה לפי ל-10,000" : "צביעה לפי כמות";
      const rangeText = (currentMetric === "rate")
        ? `טווח: <b>${minVal.toLocaleString()}</b> עד <b>${maxVal.toLocaleString()}</b> /10k`
        : `טווח: <b>${minVal.toLocaleString()}</b> עד <b>${maxVal.toLocaleString()}</b> אירועים`;

      showInfo(
        metricTitle,
        `ירוק = נמוך • כתום = בינוני • אדום = גבוה<br>${rangeText} (ישובים מעל המקום ה-2 מוצגים באדום מקסימלי)${noj}${extra}`,
        "info"
      );
    }

    // ✅ קליק על צביעה: GovMap מחזיר אירוע דרך progress עם data.name (שם הגיאומטריה)
    function handleChoroplethClick(ev, nameToMuniMap) {
      try {
        ...
        const municipalityName = nameToMuniMap && nameToMuniMap[geomName] ? nameToMuniMap[geomName] : null;
        if (!municipalityName) return;

        const key = normalizeKey(municipalityName);
        const entry = muniIndex[key] || muniIndex[nameVariants(municipalityName)[0]];

        if (!entry) {
          showInfo("לא נמצאו נתונים", `אין נתוני פשיעה עבור <strong>${escapeHtml(municipalityName)}</strong>`, "warn");
          return;
        }
        ...
        console.warn("handleChoroplethClick failed", e);
      }
    }
    ...
    async function initGovMap() {
      ...
      const stationsDS = getStationsDataset();
      if (!stationsDS) console.warn("⚠️ crimeByYearStations.js לא נטען / או שהמשתנה לא נקרא נכון (מצפה ל-window.crimeByYearStations או crimeByYearStations).");

      buildPopulationIndex();
      updateMetricButtonsUI();

      // כפתור תחנות — ננעל אם אין דאטה
      const btnStation = document.getElementById('btnModeStation');
      if (btnStation) {
        const hasStations = !!getStationsDataset();
        btnStation.classList.toggle('disabled', !hasStations);
        btnStation.disabled = !hasStations;
        if (!hasStations && currentMode === 'station') currentMode = 'muni';
      }
      ...
      yearlyStats[`merhav_${currentYear}`] = calculateStats(currentYear, "merhav");
      yearlyStats[`station_${currentYear}`] = calculateStats(currentYear, "station");
      yearlyStats[`${currentMode}_${currentYear}`] = calculateStats(currentYear, currentMode);
      updateQuickStats();
      ...
      sel.addEventListener("change", async e => {
        ...
        if (currentTab === "compare") updateCompareView();
          ...
          const tc = document.getElementById("trends-content");
          if (tc) tc.innerHTML = `<div class="message message-info">בחר ${entityLabelByMode(currentMode)} במפה כדי לראות נתונים היסטוריים</div>`;
        } else if (currentTab === "rankings") renderRankings();
        else {
          showInfo(
            "מפה לפי " + modeLabel(currentMode),
            `שנה נבחרת: ${currentYear}<br>לחץ על ${entityLabelByMode(currentMode)} במפה כדי לראות נתונים`,
            "info"
          );
        }
      });
      ...
      govmap.createMap("map", {
        ...
        onLoad: () => {
          ...
          // ✅ מאזין יחיד ללחיצות — נשאר תמיד, גם בזמן צביעה
          ...
          showInfo("מפה לפי " + modeLabel(currentMode), `שנה נבחרת: ${currentYear}<br>לחץ על ${entityLabelByMode(currentMode)} במפה כדי לראות נתונים`, "info");
    ...
    <div id="info-content">
      ...
      <div class="tab-content" id="tab-compare">
        <div id="compare-content">
          <div class="message message-info">בחר ישויות במפה והוסף להשוואה</div>
        </div>
      </div>

      <div class="tab-content" id="tab-trends">
        <div id="trends-content">
          <div class="message message-info">בחר ישות במפה לצפייה בהיסטוריה</div>
        </div>
      </div>
  ...
  <div id="aboutModal" class="modal" aria-hidden="true">
    ...
    <div class="modalCard" role="dialog">
      ...
      <div class="modalBody">
        ...
        <ul>
          <li><strong>נתוני פשיעה:</strong> מעובדים ממאגרי מידע משטרתיים (2020-2025) + תחנות (שכבה 24).</li>
          <li><strong>אוכלוסייה:</strong> נתוני למ"ס (2019) לחישוב יחס ל-10,000 נפש (רק ברשויות).</li>
        </ul>
